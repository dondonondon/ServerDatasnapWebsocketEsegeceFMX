...
-(DSRESTCommand *) CreateCommand{
	return[[[DSRESTCommand alloc]initWithConnection:self]autorelease];
...
-(NSString *) buildRequestURL:(DSRESTCommand *) command{
	...
	if ((command.RequestType == GET)
		...
		NSRange replaceRange2 =[LMethodName rangeOfString:LMethodName];
		[LMethodName replaceOccurrencesOfString:@"\\" withString: @"\%22" options:NSCaseInsensitiveSearch range:replaceRange2];
		[LMethodName appendString: [NSString stringWithString:@"%22"]];
	}
	...
	switch (arequestType) {
		...
		case POST:
			...
			if ([_parameters count] > 0) {
				...
				[jbody retain];
				@try {	
					TJSONObject * jo=   [[[TJSONObject alloc]initWithJSONObject:jbody]autorelease];
		...
		for (  NSString * e in ds){
			...
			if([er count]>0){
				...
				if ([s isEqualToString:@"dssession"]) {
					[SessionID release];
					SessionID = [[er objectAtIndex:1] retain]  ;
					found = YES;
				}else{
...
-(void) execute: (DSRESTCommand *) command{
	...
	@try {
		...
		@try{		
			...
			NSString * json_string1 =[[[NSString alloc] initWithData:response encoding:NSUTF8StringEncoding] autorelease];
			id json1 = [[[TJSONObject alloc] initWithJSONString:json_string1]autorelease];
			...
			@try{
				...
				id json = [[[TJSONObject alloc] initWithJSONString:json_string]autorelease];
