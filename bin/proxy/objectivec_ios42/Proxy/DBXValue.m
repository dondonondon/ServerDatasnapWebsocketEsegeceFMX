...
-(NSString *) toString {
	...
	if ([ self containsASimpleValueType])
		  ...
		  return [[[[ TJSONNull alloc ] init] autorelease ] asJSONString];
      }else{
...
-(void) dealloc {
	[stringValue release];
	[JSONGenericElement release];
    [DBXValueValue release];
	[JSONValueValue release];
...
-(void) SetAsDateTime:(NSDate *) value {
	[DateTimeValue release];
	DateTimeValue =[value retain];
	[self setDBXType:DateTimeType];
}
-(void) SetAsTimeStamp:(NSDate *) value {
	[TimeStampValue release];
	TimeStampValue =[value retain];
