...
+(id) getInstance{
	@synchronized (self) {
		if (!istance){
			istance = [[[DBXDefaultFormatter alloc] init] autorelease] ;}
...
-(NSString*) Int32ToString:(long) value { return [NSString stringWithFormat:@"%i", value]; }	
...
-(NSString*) UInt32ToString:(long) value { return [NSString stringWithFormat:@"%qi", value]; }
...
-(int) StringToTDBXTime: (NSString *) value {	
	...
	[tt release];
	NSDateComponents *h = [gregorian components:NSHourCalendarUnit
										   fromDate:dt];	
	NSDateComponents *m = [gregorian components:NSMinuteCalendarUnit
										   fromDate:dt];	
    NSDateComponents *s = [gregorian components:NSSecondCalendarUnit
										   fromDate:dt];		
