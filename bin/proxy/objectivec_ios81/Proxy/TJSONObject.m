...
-(TJSONPairList *) buildElements:(NSDictionary * )o {
	@try {
		...
		TJSONPairList* res =[[TJSONPairList alloc] init]  ;
		for (NSString * pname in o) {
			id obj = [o objectForKey:pname];
			if ([obj isKindOfClass:[NSNull class]]) {
				[res addPair: [TJSONPair JSONPairWithName:pname andValue:[[TJSONNull alloc]init]]];
			} else if ([obj isKindOfClass:[NSString class]]) {
				[res addPair: [TJSONPair JSONPairWithName:pname andValue:[[TJSONString alloc]initWithString:(NSString *)obj]]];
			} else if ([obj isKindOfClass:[NSArray class]]) {
				[res addPair: [TJSONPair JSONPairWithName:pname andValue:[[TJSONArray alloc]initWithJSONArray:obj]]];
			}else if ([obj isKindOfClass:[TJSONObject class]]) {
				[res addPair:[TJSONPair JSONPairWithName:pname andValue:obj]];
			}else if ([obj isKindOfClass:[NSDictionary class]]) {
				[res addPair: [TJSONPair JSONPairWithName:pname andValue:[[TJSONObject alloc]initWithJSONObject:obj]]];
			} else if ([obj isKindOfClass:[NSNumber class]]) {
				if (strcmp([obj objCType], @encode(BOOL)) == 0){
					if ([obj boolValue]) {
						[res addPair: [TJSONPair JSONPairWithName:pname andValue:[[TJSONTrue alloc]init]]];}
					else{
						[res addPair: [TJSONPair JSONPairWithName:pname andValue:[[TJSONFalse alloc]init]]];}
				}else{
					[res addPair: [TJSONPair JSONPairWithName:pname andValue:[[TJSONNumber alloc]initWithNumber:obj]]];}
			}			
...
-(id) initWithJSONString:(NSString *) stringJson{
	...
	self = [self init];
	if (self) {
		elements  = [self buildElements: [self JSONObjectFromString:stringJson]];
...
if (value) {
	[elements addPair:[TJSONPair JSONPairWithName:name andValue:[[TJSONTrue alloc]init]]];
}else{
	[elements addPair:[TJSONPair JSONPairWithName:name andValue:[[TJSONFalse alloc]init]]];
}
	return self;
}
...
+(id)parse:(NSString *) value{
	return [[TJSONObject alloc]initWithJSONString:value];
}
@end
