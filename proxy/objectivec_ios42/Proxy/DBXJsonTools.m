...
+(TJSONValue *) JSONToJSONValue:(id) o{
	if ([o isKindOfClass: [NSNull class]]) // TJSONNull
		return [[[TJSONNull alloc] init]autorelease];
	if ([o isKindOfClass: [NSDictionary class]]){ // TJSONObject
		return [[[ TJSONObject alloc] initWithJSONObject: o]autorelease ];}
	if ([o isKindOfClass: [NSArray class]]){ // TJSONArray
		return [[[ TJSONArray alloc] initWithJSONArray: o]autorelease] ;}
	if ([o isKindOfClass:[NSNumber class]]){
		if ([o boolValue]){
	 		return [[[TJSONTrue alloc]init]autorelease];		}
		else {
			return [[[TJSONFalse alloc]init]autorelease];	
		}
	}
...
+(id) JSONToTableType:(id) value withDBXType: (NSString*) DBXTypeName{
	if ([DBXTypeName isEqualToString:@"TParams"]) {
		return [TParams paramsWithJSON: [[[TJSONObject alloc] initWithJSONObject:value]autorelease]];
	} else {
		if ([DBXTypeName isEqualToString:@"TDBXReader"]||
			[DBXTypeName isEqualToString:@"TDBXReaderValue"]) {
			return [TDBXReader  DBXReaderWithJSON: [[[TJSONObject alloc] initWithJSONObject:value]autorelease]];
		} else if ([DBXTypeName isEqualToString:@"TDataSet"]) {
			
			return [TDataSet DataSetWithJSON: [[[TJSONObject alloc] initWithJSONObject:value]autorelease]];
		}
	}
	@throw [DBXException
				...
				switch ([*value DBXType]) {
					...
					case UInt64Type:
						...
						if ([dbxTypeName isEqualToString:@"TDBXStreamValue"]) {
							[[*value GetAsDBXValue]  SetAsStream:[TStream streamWithJsonArray:[[[TJSONArray alloc] initWithJSONArray:obj]autorelease]]];	
						}else {
							
						
							[*value SetAsStream:[TStream streamWithJsonArray:[[[TJSONArray alloc] initWithJSONArray:obj]autorelease]]];	
						}
							break;
						case BlobType:
						    
						    [*value SetAsBlob:[TStream streamWithJsonArray:[[[TJSONArray alloc] initWithJSONArray:obj]autorelease]]];
							break;
					case TimeStampType:{
...
+(DBXValueType *) JSONToValueType:(TJSONArray *) json {
	DBXValueType * vt = [[[DBXValueType alloc]init]autorelease];
...
+(id) DBXParametersToJSONObject:(TParams*) dbxParameters {
	...
	NSMutableArray * arr2 = [[[NSMutableArray alloc]initWithCapacity:1]autorelease];
	...
	for (int i = 0; i < [dbxParameters size]; i++) {
		NSMutableArray * arr3 = [[[NSMutableArray alloc]initWithCapacity:1]autorelease];
...
+(TJSONObject *) DBXReaderToJSONObject: (TDBXReader*) dbxReader {
	...
	NSMutableDictionary * json = [[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];
	NSMutableArray * arrayParams;
	TParams* columns = [dbxReader columns];
	arrayParams = [[[NSMutableArray alloc]initWithCapacity:0]autorelease ];
	for (int i = 0; i < [columns size]; i++) {
		[arrayParams addObject:[[columns getParamByIndex:i]toJson]];
		// Create the empty JSONArray for the data. Will be filled after
		[json setObject:[[[NSMutableArray alloc]initWithCapacity:0]autorelease] forKey:[columns getParamByIndex:i].name];
	}
	...
	return  [[[TJSONObject alloc]initWithJSONObject: json]autorelease];
...
+(TJSONArray *) ValueTypeToJSON:(DBXValueType *) dataType {
	...
	dataType.valueParameter,
					  [NSNumber numberWithInt:dataType.literal], nil];
	return  [[[TJSONArray alloc]initWithJSONArray: Meta]autorelease ];
}
